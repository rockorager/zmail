...
pub fn wait(self: *Login) imap.Error!void {
    while (self.result == null) {
        self.cond.wait(&self.mutex);
    }
    switch (self.result.?) {
        .ok => return,
        .no => return error.LoginFailed,
        .bad => return error.InvalidArgumentsOrCommandUnknown,
    }
}

pub fn command(self: *Login) Command {
    return .{
        .ptr = self,
        .writeTo_fn = writeTo,
...
fn writeTo(ptr: *anyopaque, writer: std.io.AnyWriter) !void {
    const self: *Login = @ptrCast(@alignCast(ptr));
    try writer.print("{x} {s} {s} {s}\r\n", .{ self.tag, name, self.username, self.password });
    self.mutex.lock();
}
